...
bool mk::bag::detail::parse_record(unsigned char const* const& data, std::uint64_t const& len, std::uint64_t& idx, mk::bag::record_t* const& out_record)
{
	...
	static constexpr auto const s_field_callback = [](void* const& ctx, callback_variant_e const& variant, void const* const& data) -> bool
	{
		fields_callback_ctx_t& fields_callback_ctx = *static_cast<fields_callback_ctx_t*>(ctx);
		CHECK_RET(variant == callback_variant_e::field, false);
		field_t const& field = *static_cast<field_t const*>(data);

		CHECK_RET(fields_callback_ctx.m_fields_count < s_fields_max, false);
	...
	bool const fields_parsed = parse_fields(data, len, idx, header_len, s_field_callback, &fields_callback_ctx);
...
bool mk::bag::parse_records(void const* const& void_data, std::uint64_t const& len, callback_t const& callback, void* const& callback_ctx)
{
	...
	while(idx != len)
	{
		...
		CHECK_RET(called_back, false);
	}
	return true;
}

bool mk::bag::parse_fields(void const* const& void_data, std::uint64_t const& len, std::uint64_t& idx, std::uint32_t const& header_len, callback_t const& callback, void* const& callback_ctx)
{
	assert(callback);
	unsigned char const* const data = static_cast<unsigned char const*>(void_data);
	...
	{
		field_t field;
		bool const field_parsed = detail::parse_field(data, len, idx, header_len, header_idx, &field);
